...
export const useNoteStore = create<NoteState>()(
  subscribeWithSelector(
    persist(
      (set, get) => {
        const unpinOtherNotes = async (excludeId?: string) => {
          const pinnedNotes = get().notes.filter((n) => n.pinned && n.id !== excludeId);
          if (pinnedNotes.length === 0) return;

          await Promise.all(
            pinnedNotes.map((n) =>
              db.notes.update(n.id, { pinned: false, updatedAt: Date.now() })
            )
          );

          set((state) => ({
            notes: state.notes.map((n) =>
              n.pinned && n.id !== excludeId
